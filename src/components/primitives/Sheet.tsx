...
const sideClass: Record<NonNullable<SheetProps['side']>, string> = {
  ...
  responsive:
    ...
    // Chat/people panel on a pointer device. Narrower on small laptops (it ate
    // ~40% of an md viewport at 22rem) and steps up only on large screens.
    'md:inset-x-auto md:right-3 md:top-3 md:w-[min(92vw,19rem)] md:max-h-none md:rounded-island md:pb-0 ' +
    // Below `xl` the panel STOPS ABOVE the control bar — 5.75rem clears the bar's
    // 1rem inset, its 62px island and the usual 12px gutter. That is what keeps
    // the bar whole and motionless at those widths: it has nothing to dodge, so
    // it never slides a Leave control under a resting cursor (lib/panelDock).
    // Squeezing the two side by side doesn't work there anyway — the bar is
    // 614px and an md viewport only has 768px to give.
    'md:bottom-[5.75rem] ' +
    // Below `lg` it also OVERLAYS: the stage keeps its full width and the panel
    // floats over it. From `lg` the stage reflows for it (RoomView).
    'lg:w-[21rem] ' +
    // Only at `xl` is there room for the panel to run full-height BESIDE the bar.
    // This width is mirrored in lib/panelDock's collidingPanelWidth, which the
    // control bar measures its clearance against — change both together.
    'xl:bottom-3 xl:w-[24rem]',
}
