...
import { isSealed, newDeviceKeyPair, openSealed, sealFor } from './sealedSecrets'
...
describe('sealedSecrets', () => {
  it('only the addressed devices can open it, and each gets the secrets back', async () => {
    ...
    expect(await openSealed(sealed!, 'dev-a', a.privateKey)).toEqual({ secret: 'join-123', e2ee: 'key-xyz' })
    expect(await openSealed(sealed!, 'dev-b', b.privateKey)).toEqual({ secret: 'join-123', e2ee: 'key-xyz' })
    // Not addressed to it, or the wrong key for an address it claims.
    expect(await openSealed(sealed!, 'dev-c', c.privateKey)).toBeNull()
    expect(await openSealed(sealed!, 'dev-a', c.privateKey)).toBeNull()
  })
  ...
  it('seals nothing without devices or secrets, and refuses junk', async () => {
    ...
    expect(await openSealed('sealed1:!!', 'd', a.privateKey)).toBeNull()
    expect(await openSealed('plain-key', 'd', a.privateKey)).toBeNull()
  })

  it('keeps an absent half absent', async () => {
    const a = await newDeviceKeyPair()
    const sealed = await sealFor([{ deviceId: 'd', publicJwk: a.publicJwk }], { e2ee: 'only-key' })
    expect(await openSealed(sealed!, 'd', a.privateKey)).toEqual({ secret: undefined, e2ee: 'only-key' })
  })
})
